...
BOOL kActivateKeyboard(void)
{
	...
	// 0xFFFF만큼(임의의 값) 루프를 수행할 시간이면 충분한 커맨드가 전송될 수 있음
	...
	// 입력 버퍼(포트 0x60)로 키보드 활성화(0xFA) 커맨드를 전달하여 키보드로 전송 
	// 키보드 활성화 
	...
	for(j=0;j<100;j++)
	{
		...
		// 0xFFFF만큼(임의의 값) 루프를 수행할 시간이면 충분한 커맨드의 응답이 올 수 있음 
...
BOOL kChangeKeyboardLED(BOOL bCapsLockOn,BOOL bNumLockOn,BOOL bScrollLockOn)
{
	...
	// 키보드에 LED 변경 커맨드(LED 변경될것이라고 알리는 커맨드)를 전송하고 커맨드가 처리될 떄까지 대기 
...
void kEnableA20Gate(void)
{
	...
	// 출력 포트(포트 0x60)에 수신되니 키보드 컨트롤러의 출력 포트 값을 읽음
...
// 스캔 코드가 알파벳 범위인지 여부를 확인
BOOL kIsAlphabetScanCode(BYTE bScanCode)
{
	// 변환 테이블의 값을 직접 읽어서 알파벳 범위인지 확인
...
// 숫자 또는 기호 범위인지 여부를 확인
BOOL kIsNumberOrSymbolScanCode(BYTE bScanCode)
{
	// 숫자 패드나 확장 키 범위를 제외한 범위(스캔 코드 2~53)에서 영문자가 아니면 숫자나 기호 또는 don't care임
...
// 동기화 처리가 된 이유는 만약 프로세서 1에서 keyboardinterrupt의 kPutQueue()을 실행하고 , 프로세서 2에서 
// 셸을 실행하여 kGetQueue()을 동시에 실행할 경우 동기화 문제가 발생할 수 있다 !!!
