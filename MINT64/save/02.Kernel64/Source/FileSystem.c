...
static BOOL kFreeClusterUtilEnd(DWORD dwClusterIndex);
...
BOOL kMount(void)
{
	...
	// 시그니처를 확인하여 같다면 자료구조에 각 영역에 대한 정보 삽입 
	pstMBR = (MBR*)gs_vbTempBuffer;

	if(pstMBR->dwSignature != FILESYSTEM_SIGNATUR)
	{
		// 동기화 처리 
		kUnlock(&(gs_stFileSystemManager.stMutex));
		return FALSE;
...
static DWORD kFindFreeCluster(void)
{
	...
	for(i=0;i<gs_stFileSystemManager.dwClusterLinkAreaSize;i++)
	{
		...
		// 이번에 읽어야할 클러스터 링크 테이블의 섹터 오프셋을 구해서 읽음 
...
static BOOL kSetClusterLinkData(DWORD dwClusterIndex,DWORD dwData)
{
	...
	{
		kPrintf("kReadClusterLinkTable error\n");
		return FALSE;
	}
	...
	{
		kPrintf("kWriteClusterLinkTable error\n");
		return FALSE;
	}
...
static BOOL kGetClusterLinkData(DWORD dwClusterIndex,DWORD* pdwData)
{
	...
	// 한 섹터에 128개의 클러스터 링크가 들어가므로 128로 나누면 섹터 오프셋으르 구할 수 있음 
	dwSectorOffset = dwClusterIndex/128;
...
FILE* kOpenFile(const char* pcFileName,const char* pcMode)
{
	...
	else if(pcMode[0]=='w')
	{
		...
		if(kFreeClusterUtilEnd(dwSecondCluster)==FALSE)
		{
			// 동기화 
			kUnlock(&(gs_stFileSystemManager.stMutex));
			return NULL;
...
DWORD kWriteFile(const void* pvBuffer,DWORD dwSize,DWORD dwCount,FILE* pstFile)
{
	...
	{
		dwUpperFileSize=FILESYSTEM_CLUSTERSIZE;
	}
	else
	{
		dwUpperFileSize = ((pstFileHandle->dwFileSize+FILESYSTEM_CLUSTERSIZE-1)/FILESYSTEM_CLUSTERSIZE)*FILESYSTEM_CLUSTERSIZE;
	}
	...
	{	
		...
		{
			if((pstFileHandle->dwCurrentClusterIndex==FILESYSTEM_LASTCLUSTER)||(pstFileHandle->dwCurrentClusterIndex==pstFileHandle->dwPreviousClusterIndex))
			{
...
int kSeekFile(FILE* pstFile,int iOffset,int iOrigin)
{
	...
	//========================================================================================
	// Origin과 Offset을 조합하여 파일 시작을 기준으로 파일 포인터를 옮겨야할 위치 계산
	//========================================================================================
	// 옵션에 따라서 실제 위치를 계산 
	// 음수면 파일의 시작방향으로 이동하며, 양수면 파일의 끝 방향으로 이동 
	switch(iOrigin)
	{
		// 파일의 시작을 기준으로 이동 
