...
void kInitializeSerialPort(void)
{
	...
	// 통신 속도를 115200으로 설정 
	// 라인 제어 레지스터(포트 0x3FB)의 DLAB 비트(비트 7)를 1로 설정하여 제수 래치 레지스터에 접근 
	...
	// 송수신 방법을 설정 
	// 라인 제어 레지스터(포트 0x3FB)에 통신 방법을 8비트, 패리티 없음
	// 1 Stop 비트로 섲렁하고, 제수 래치 레지스터 사용이 끝났으므로 DLAB 비트를 0으로 설정 
...
int kReceiveSerialData(BYTE* pbBuffer,int iSize)
{
	...
	for(i=0;i<iSize;i++)
	{
		...
		pbBuffer[i] = kInPortByte(SERIAL_PORT_COM1+SERIAL_PORT_INDEX_RECEIVERBUFFER);
	}
