...
WORD kReadCounter0(void)
{
	...
	kOutPortByte(PIT_PORT_CONTROL,PIT_CONTER0_LATCH);

	// 카운터 0(포트 0x40)에서 LSB->MSB 순으로 카운트 값을 읽음
...
// 함수를 호출하면 PIT 컨트롤의 설정이 바뀌므로, 이후에 PIT 컨트롤러를 재설정해야 함
// 정확하게 측정하려면 함수 사용 전에 인터럽트를 비활서오하 하는 것이 좋음
// 약 50ms까지 측정 가능
void kWaitUsingDirectPIT(WORD wCount)
{
	WORD wLastCounter0;
	WORD wCurrntCounter0;
	...
	{
		// 현재 카운터 0의 값을 반한
		wCurrntCounter0=kReadCounter0();
		if(((wLastCounter0-wCurrntCounter0)&0xFFFF)>=wCount)
		{
			break;
		}
	}
