...
; 외부에서 정의된 함수를 쓸 수 있도록 선엄함(Import)
...
%macro KLOADCONTEXT 0   ; 파라미터를 전달받지 않는 KLOADCONTEXT 매크로 정의 
    ...
    pop fs
    pop rax
    mov es, ax      ; ES 세그먼트 셀렉터와 DS 세그먼트 셀렉터는 스택에 직접 꺼내 복원할 수 없으므로 RAX 레지스터에 저장한 뒤에 복원 
    pop rax         
    mov ds, ax
...
; #0, Divide Error ISR
kISRDivideError:
    KSAVECONTEXT    ; 콘텍스트를 젖아한 뒤 셀렉터를 커널 데이터 디스크립터로 교체 
    ...
    iretq           ; 인터럽트 처리를 완료하고 이전에 수행되던 코드로 복원 
...
; #19, SIME Floating Point Exception ISR
kISRSIMDError:
    KSAVECONTEXT    ; 콘텍스트를 젖아한 뒤 셀렉터를 커널 데이터 디스크립터로 교체 
