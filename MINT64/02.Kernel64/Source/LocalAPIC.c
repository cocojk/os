...
void kEnableSoftwareLocalAPIC(void)
{
	...
	// MP 설정 테이블 헤더에 저장된 로컬 APICdㅢ 메모리 맵 I/O 어드레스를 사용 
	qwLocalAPICBaseAddress = kGetLocalAPICBaseAddress();

	// 의사 인터럽트 벡터 레지스터(Spurious Interrupt Vector Register)의 
	// APIC 소프트웨어 활성/비활성 필드(비트8)를 1로 설정해서 로컬 APIC를 활성화 
...
// 태스크 우선순위 레지스터(Task Prioirty Register) 설정 
