...
// 동적 메모리 영역 초기화 
// 전체 동적 메모리 크기에서 메타 데이터를 구한다. 
// 메타 데이터를 제외한 크기에 대해서 메타 데이터를 구하는 것이 정확한 방법이지만,
// 전체 동적 메모리 크기에 대한 메타 데이터를 구함으로써 간접적으로 처리한다.
// 실제 필요한 메타 데이터의 크기 << 현재 할당된 메타 데이터 크기 이다. 
void kInitializeDynamicMemory(void)
{
	QWORD qwDynamicMemorySize;
	int i,j;
	BYTE* pbCurrentBitmapPosition;
	int iBlockCountOfLevel, iMetaBlockCoount;

	// 동적 메모리 영역으로 사용할 메모리 크기를 이용하여 블록을 관리하는데 
	// 필요한 메모리 크기를 최소 블록 단위로 계산 
	qwDynamicMemorySize = kCalculateDynamicMemorySize();
	iMetaBlockCoount = kCalculateMetaBlockCount(qwDynamicMemorySize);

	// 전체 블록 개수에서 관리에 필요한 메타 블록의 개수를 제외한 나머지 영역에 대해서 
	// 메타 정보를 구성 
	gs_stDynamicMemory.iBlockCountOfSmallestBlock = (qwDynamicMemorySize/DYNAMICMEMORY_MIN_SIZE)-iMetaBlockCoount;
	...
	gs_stDynamicMemory.qwStartAddress = DYNAMICMEMORY_START_ADDRESS+iMetaBlockCoount*DYNAMICMEMORY_MIN_SIZE;
...
void* kAllocateMemory(QWORD qwSize)
{
	...
	if(qwAlignedSize == 0)
	{
	//	kPrintf("kGetBuddyBlockSize error\n");
		return NULL;
	}
...
//		kPrintf("not enough size\n");
	...
	if(lOffset==-1)
	{
//		kPrintf("kAllocationBuddyBlock error\n");
		return NULL;
	}
...
static int kAllocationBuddyBlock(QWORD qwAlignedSize)
{
	...
	{
//		kPrintf("kGetBlockListIndexOfMatchSize error\n");
		return -1;
	}
...
BOOL kFreeMemory(void* pvAddress)
{
	...
	// 넘겨받은 어드레스를 블록 풀을 기준으로 하는 어드레스로 변환하여 할당했던 블록의 크기를 검색 
	...
	// 버디 블록의 최소 크길르 블록 리스트 인덱스로 시프트하여 할당된 블록의 크기 계산 
