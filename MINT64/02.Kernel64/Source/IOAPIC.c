...
// ISA 버스가 연결된 I/O APIC의 기준 어드레스르 ㄹ반환 
QWORD kGetIOAPICBaseAddressOfISA(void)
{
	...
	if(gs_stIOAPICManager.qwIOAPICBaseAddrressOfISA == NULL)
	{
		pstIOAPICEntry = kFindIOAPICEntryForISA();
		if(pstIOAPICEntry !=NULL)
		{
			gs_stIOAPICManager.qwIOAPICBaseAddrressOfISA = pstIOAPICEntry->dwMemoryMapAddress&0xFFFFFFFF;
		}
	}

	// I/O APIC의 기준 어드레스를 찾아서 저장한 다음 반환 
	return gs_stIOAPICManager.qwIOAPICBaseAddrressOfISA;
}
...
void kReadIOAPICRedirectionTable(int iINTIN,IOREDIRECTIONTABLE* pstEntry)
{
	...
	// I/O 리다이렉션 테이블은 8바이트이므로 정수로 변환해서 처리 
	pqwData = (QWORD*)pstEntry;

	//========================================================================================
	// I/O 리다이렉션 테이블의 상위 4바이트를 읽음 
	// I/O 리다이렉션 테이블은 상위 레지스터의 하위 레지스터가 한 쌍이므로 INTIN에 2를 곱하여 
...
void kRoutingIRQToAPICID(int iIRQ,BYTE bAPICID)
{
	...
	// 설정된 I/O 리다이렉션 테이블을 읽어서 목적지(Destincation) 필드만 수정 
