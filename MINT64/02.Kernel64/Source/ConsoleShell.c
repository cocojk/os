...
static void kStringToDecimalHexText(const char* pcParameterBuffer);
static void kShutdown(const char* pcParameterBuffer);
static void kSetTimer(const char* pcParameterBuffer);
static void kWaitUsingPIT(const char* pcParameterBuffer);
static void kReadTimeStampCounter(const char* pcParameterBuffer);
static void kMeasurPrecessorSpeed(const char* pcParameterBuffer);
...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
	...
	{"totalram","Show TotalRAM Size",kShowTotalRAMSize},
	{"strtod","String to Decimal/Hex convert, ex) strtod 1000(decimal) strtod 0x10(hex)",kStringToDecimalHexText},
	{"shutdown","Shutdown And Reboot OS",kShutdown},
	{"settimer","Set PIT Controller Counter0, ex)settimer 10(ms) 1(periodic true(1)/false(0))",kSetTimer},
	{"wait","Wait ms Using PIT, ex)wait 100(ms)",kWaitUsingPIT},
	{"rdtsc","Read Time Stamp Counter",kReadTimeStampCounter},
	{"cpuspeed","Measure Processor Speed",kMeasurPrecessorSpeed},
...
{
	char vcCommanBuffer[CONSOLESHELL_MAXCOMMANDBUFFERCOUNT];
	int iCommanBufferIndex=0;
	BYTE bKey;
	int iCursorX, iCursorY;

	// 프롬프트 출력
	kPrintf(CONSOLESHELL_PROMPRTMESSAGE);
	...
	while(1)
	{
		...
		else if(bKey == KEY_ENTER)
		{
			...
			{
				// 커맨드 버퍼에 있는 명령을 실행
				vcCommanBuffer[iCommanBufferIndex]='\0';
				kExecuteCommand(vcCommanBuffer);
			}

			// 프롬프트 출력 및 커맨드 버퍼 초기화
			kPrintf("%s",CONSOLESHELL_PROMPRTMESSAGE);
			kMemSet(vcCommanBuffer,'\0',CONSOLESHELL_MAXCOMMANDBUFFERCOUNT);
			iCommanBufferIndex=0;
		}
		...
		else
		{
			...
			// 버퍼에 공간이 남아 있을 때만 가능 버그 유발 가능성 매우 높다
			if(iCommanBufferIndex<CONSOLESHELL_MAXCOMMANDBUFFERCOUNT)
			{
				vcCommanBuffer[iCommanBufferIndex++]=bKey;
				kPrintf("%c",bKey);
			}
		}
	}
...
// 커맨드 버퍼에 있는 커맨드를 비교하여 해당 커맨드를 처리하는 함수를 진행
...
static void kHelp(const char* pcParameterBuffer)
{
	...
	for(i=0;i<iCount;i++)
	{
		...
		if((i!=0)&&((i%20)==0))
		{
			kPrintf("Pree any Key to continue... ('q' is exit) :");
			if(kGetCh()=='q')
			{
				kPrintf("\n");
				break;
...
static void kStringToDecimalHexText(const char* pcParameterBuffer)
{
	...
	while(1)
	{
		...
		// 파라미터에 대한 정보를 출력하고 16진수인지 10진수인지 판단하여 반환 후 결과를 printf로 출력
		kPrintf("Param %d = '%s', Length = %d, ",iCount+1,vcParameter,iLength);

		// 0x로 시작하면 16진수, 그외에느 10진수로 판단
		if(kMemCmp(vcParameter,"0x",2)==0)
		{

			iValue=kAToI(vcParameter+2,16);
			kPrintf("HEX value = %q\n",iValue);
		}
		else
		{
			iValue=kAToI(vcParameter,10);
...
static void kWaitUsingPIT(const char* pcParameterBuffer)
{
	...
	lMilisecond = kAToI(vcParameter,10);
...
static void kShowDateAndTime(const char* pcParameterBuffer)
{
	...
	kReadRTCData(&wYear,&bMonth,&bDayOfMonth,&bDayOFWeek);
...
static void kTestTask1(void)
{
	...
	iMargin = (pstRunningTask->stLink.qwID&0xFFFFFFFF)%10+1;
...
static void kShowTaskList(const char* pcParameterBuffer)
{
	...
	// 코어 수만큼 루프를 돌면서 각 스케줄러에 있는 태스크의 수를 반환 
	...
	for(i=0;i<TASK_MAXCOUNT;i++)
	{
		...
		{
			// 태스크가 6개 출력될 때마다 태스크 정보를 표시할지 여부를 확인 
...
static void kPrintNumberTask(void)
{
	...
	// 콘솔 화면이 꽉차서 화면을 한칸씩 올리는 작업이 다 완료될때까지 기다림.
...
static void kShowMatrix(const char* pcParameterBuffer)
{
	...
	if(pstProcess!=NULL)
	{
		kPrintf("Matrix Process [0x%Q] CreateSuccess\n",pstProcess->stLink.qwID);
...
static void kShowRootDirectory(const char* pcParameterBuffer)
{
	...
	// 먼저 루프를 돌면서 디렉터링레 있는 파일의 개수와 전체 파일이 사용한 크기를 계산 
	iTotalCount=0; 
	dwTotalByte=0;
	...
	while(1)
	{
		...
		// 파일의 시작 클러스터 삽입 
		kSPrintf(vcBuffer+55,"0x%X Cluster",pstEntry->dwStartClusterIndex);
		kPrintf("	%s\n",vcBuffer);
...
static void kReadDataFromFile(const char* pcParameterBuffer)
{
	...
	while(1)
	{
		...
		if(bKey==KEY_ENTER)
		{
			...
			{
				kPrintf("Press any ket to continue... ('q' is exit) :");
				if(kGetCh()=='q')
				{
					kPrintf("\n");
					break;
...
static void kTestFileIO(const char* pcParameterBuffer)
{
	...
	// w 옵션은 파일을 생성하므로 정상적으로 핸들을 반환되어야 함 
	...
	// 맨 마지막으로 이동하여 1바이트를 써서 파일의 크기를 4MB로 만듬 
