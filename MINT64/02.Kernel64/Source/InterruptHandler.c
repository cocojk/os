...
// 인터럽트 부하 분산 처리 
void kProcessorLoadBalancing(int iIRQ)
{
	...
	// 코어의 개수를 구해서 루프를 수행하여 인터럽트 처리 횟수가 가장 적은 코어를 선택 
	...
	for(i=0;i<iCoreCount;i++)
	{
		...
		// 전체 카운트가 거의 최댓값에 근접했담녀 나중에 카운트를 모두 0으로 설정 
	...
	// 처리한 코어의 카운트가 최댓값에 근접했담녀 전체 카운트를 다시 0에서 시작하도록 변경 
	if(bResetCount==TRUE)
	{
		for(i=0;i<iCoreCount;i++)
		{
...
void kKeyboardHandler(int iVectorNumber)
{
	...
	// 키보드 컨트롤러에서 데이러르 읽어서 ASCII로 변환하여 큐에 삽입
...
// 모든 interrupt는 nested 불가능 따라서, 해당 functiond에서는 동기화 처리 안해줘도 된다.
