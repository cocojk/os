...
void kInitializeQueue(QUEUE* pstQueue,void* pvQueueBuffer,int iMaxDataCount,int iDataSize)
{
	...
	// 큐의 삽입 위치와 제거 위치를 초기화하고 마지막으로 수행된 명령을 제거로 설정하여 큐를 빈 상태로 만듬
	pstQueue->iPutIndex=0;
	pstQueue->iGetindex=0;
	pstQueue->bLastOpereationPut=FALSE;
}

// 큐가 가득 찼는지 여부를 반환
BOOL kIsQueueFull(const QUEUE* pstQueue)
{
	// 큐의 삽입 인덱스와 제거 인덱스가 같고 마지막으로 수행된 명령이 삽입이면 큐가 가드 찼으므로 삽입할 수 없음
	if((pstQueue->iGetindex==pstQueue->iPutIndex)&&(pstQueue->bLastOpereationPut==TRUE))
	{
		return TRUE;
	}
	return FALSE;
...
BOOL kIsQueueEmpty(const QUEUE* pstQueue)
{
	...
	if((pstQueue->iGetindex==pstQueue->iPutIndex)&&(pstQueue->bLastOpereationPut==FALSE))
	{
		return TRUE;
	}
	return FALSE;
...
BOOL kPutQueue(QUEUE* pstQueue,const void* pvData)
{
	...
	pstQueue->bLastOpereationPut=TRUE;
	return TRUE;
}
