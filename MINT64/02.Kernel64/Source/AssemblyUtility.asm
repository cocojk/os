...
kReadTSC:
	...
	shl rdx,32					; RDX 레지스터에 있는 상위 32비트 TSC 값과 RAX 레지스터에 있는 하위 32비트 TSC 갑을 OR하여 RAX 레지스터에 64비트 TSC 값을 저장
	or rax,rdx
...
; Curent Context에 현재 콘텍스트를 저장하고 Next Task에서 콘텍스트를 복구 
...
kTestAndSet:
	...
	; RAX 레지스터에 저장된 비교할 값과 첫 번째 파라미터(rdi)의 메모리 어드레스의 값을 비교하여 
	; 두 값이 같다면 세 번째 파라미터(dl)의 값을 첫 번째 파라미터(rdi)가 가리키는 어드레스에 삽입하고 RFLAGS의 ZF비트를 1로 설정  
	; 두 값이 다르면 첫 번째 파라미터(rdi)의 값을  두 번째 파라미터(RAX)에 삽입하고 RFLAGS의 ZF 비트를 0로 설정  
	; LOCK 명령어는 어셈블리어 코드에서 전치사로 사용되며, 뒤에 따라오는 명령어를 수행하는 동안 
	; 시스템 버스를 잠그고 다른 프로세서나 코어가 메모리에 접근할 수 없게 합니다.
...
.SUCCESS:
	mov rax, 0x01			; Destination과 Compare과 같은 경우 
	ret 
