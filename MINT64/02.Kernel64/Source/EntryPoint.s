...
START:
	...
	; 최대 16개의 코어까지 지원 가능하므로 스택 영역인 1M를 16으로 나눈 값인 
	; 64Kbyte(0x10000)만큼식 아래로 이동하면서 설정 
	...
	; 추출한 APIC ID에 64Kbyte(0x10000)을 곱하여 스택의 꼭대기에 이동시킬 거리를 계산 
	mov rbx, 0x10000					; RBX 레지스터에 스택의 크기(64Kbyte)를 저장 
	mul rbx 							; RAX 레지스터에 저장된 APIC ID와 RBX 레지스터의 스택 값을 곱함 

	sub rsp, rax 						; RSP와 RBP 레지스터에서 RAX 레지스터에 저장된 값(스택의 꼭대기를 이동시킬 거리)을 
	sub rbp, rax						; 빼서 각 코어 별 스택을 할당해줌 
